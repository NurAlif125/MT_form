...
/* 52a Issuing Bank */
function toggle52a() {
  const opt = val("_090_of52a_issuing_bank");
...
/* 51a Applicant Bank */
function toggle51a() {
  const opt = val("_110_of51a_applicant_bank");
...
function toggle41a() {
  const opt = val("_170_mf41a_available_with_by");
...
/* 42a Drawee */
function toggle42a() {
  const opt = val("_190_of42a_drawee");
...
function toggle58a() {
  const opt = val("_380_of58a_requested_confirmation_party");
...
function toggle53a() {
  const opt = val("_390_of53a_reimbursing_bank");
...
function toggle57a() {
  const opt = val("_420_of57a_advise_through_bank");
...
function initMt710Form() {
  ...
  bind("_090_of52a_issuing_bank", toggle52a);
  bind("_110_of51a_applicant_bank", toggle51a);
  bind("_170_mf41a_available_with_by", toggle41a);
  bind("_190_of42a_drawee", toggle42a);
  bind("_070_mf40e_applicable_rules", toggle40e);
  bind("_380_of58a_requested_confirmation_party", toggle58a);
  bind("_390_of53a_reimbursing_bank", toggle53a);
  bind("_420_of57a_advise_through_bank", toggle57a);
...
function validateMT710() {
  ...
  const mandatoryIds = [
    ...
    "_020_mf40b_form_of_documentary_credit",
  ...
  const dateIds = ["_060_mf31c_date_of_issue", "_080_mf31d_date_of_expiry", "_280_of44c_latest_date_of_shipment"];
  ...
  const conf = val("_370_mf49_confirmation_instructions");
  if ((conf === "CONFIRM" || conf === "MAY ADD")) {
    const opt58 = val("_380_of58a_requested_confirmation_party");
    if (isEmpty(opt58)) { alert("Field 58a is required when Confirmation Instructions = CONFIRM or MAY ADD."); q("_380_of58a_requested_confirmation_party").focus(); return false; }
