...
/* 52a Issuing Bank */
function toggle52a() {
  const opt = val("_050_of52a_issuing_bank");
...
/* 42a Drawee */
function toggle42a() {
  const opt = val("_230_of42a_drawee");
...
function toggle58a() {
  const opt = val("_430_of58a_requested_confirmation_party");
...
function toggle53a() {
  const opt = val("_440_of53a_reimbursing_bank");
...
function toggle57a() {
  const opt = val("_460_of57a_advise_through_bank");
...
function initMt707Form() {
  ...
  bind("_430_of58a_requested_confirmation_party", toggle58a);
  bind("_440_of53a_reimbursing_bank", toggle53a);
  bind("_460_of57a_advise_through_bank", toggle57a);
...
function validateMT707() {
  ...
  // Date checks (YYMMDD)
  const dateIds = ["_070_mf31c_date_of_issue","_090_mf30_date_of_amendment","_320_of44c_latest_date_of_shipment","_140_of31d_date_of_expiry"];
  ...
  if (!validateStructuredField("_340_of45b_description_of_goods_and_or_services", CODE_ADDS, true)) return false;
  ...
  if (!validateStructuredField("_370_of49m_special_payment_conditions_for_beneficiary", ["ADD","DELETE","REPALL"], true)) return false;
  if (!validateStructuredField("_380_of49n_special_payment_conditions_for_bank_only", ["ADD","DELETE","REPALL"], true)) return false;

  // 71D Charges structured-light check
  if (!validate71D("_390_of71d_charges")) return false;

  // 72Z structured text rules
  if (!validate72Z("_470_of72z_sender_to_receiver_information")) return false;
...
// Call e.g. insertAdd('_340_of45b_description_of_goods_and_or_services')
