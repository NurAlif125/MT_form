            ...
            <div class="form-body tabcontents">
                <div id="view2" class="tabcontent active">
                        ...
                        <div class="form-row"><span class="label_body">MF21</span><span class="label"><a style="color:red">*</a>Receiver's Reference</span>
                            <input type="text" name="_020_mf21_receiver_reference" id="_020_mf21_receiver_reference" class="mandatory" maxlength="16" onkeypress="avoidSplChars(event)" input_type="MF21 Receiver's Reference" location="Body" value="<c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_020_mf21_receiver_reference'}"><c:out value="${item.detail}" /></c:if></c:forEach>" />
                        </div>
                        <hr/>
                        ...
                        <div class="form-row"><span class="label_body">OF25</span><span class="label">Account Identification</span>
                            <input type="text" name="_030_of25_account_identification" id="_030_of25_account_identification" maxlength="35" value="<c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_030_of25_account_identification'}"><c:out value="${item.detail}" /></c:if></c:forEach>" />
                        </div>
                        <hr/>
                        ...
                        <div class="form-row"><span class="label_body">MF30</span><span class="label"><a style="color:red;text-decoration:none">*</a> Date of Message Being Acknowledged</span>
                            <input type="text" name="_040_mf30_date_of_message_being_acknowledged" id="_040_mf30_date_of_message_being_acknowledged" class="mandatory" maxlength="6" placeholder="YYMMDD" input_type="MF30 Date of Message Being Acknowledged" location="Body" value="<c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_040_mf30_date_of_message_being_acknowledged'}"><c:out value="${item.detail}" /></c:if></c:forEach>" />
                        </div>
                        <hr/>
                        ...
                        <div class="form-row"><span class="label_body">OF32a</span><span class="label">Amount of Charges</span>
                            <select id="_050_of32a_amount_of_charges" name="_050_of32a_amount_of_charges" onchange="toggle32aOption(this.value)">
                                ...
                                <option value="B" <c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_050_of32a_amount_of_charges'}"><c:if test="${item.detail=='B'}"> selected="true" </c:if></c:if></c:forEach>>Option B (Currency + Amount)</option>
                                <option value="D" <c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_050_of32a_amount_of_charges'}"><c:if test="${item.detail=='D'}"> selected="true" </c:if></c:if></c:forEach>>Option D (Date + Currency + Amount)</option>
                            </select>
                        </div>

                        <div class="form-row" id="wrap_051_of32a_date" style="display:none;"><span class="label_body">&nbsp;</span><span class="label">Date</span>
                            <input type="text" id="_051_of32a_date" name="_051_of32a_date" maxlength="6" placeholder="YYMMDD" value="<c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_051_of32a_date'}"><c:out value="${item.detail}" /></c:if></c:forEach>" />
                        </div>
                        ...
                        <div class="form-row" id="wrap_052_of32a_currency" style="display:none;"><span class="label_body">&nbsp;</span><span class="label">Currency</span>
                            <input type="text" id="_052_of32a_currency" name="_052_of32a_currency" maxlength="3" value="<c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_052_of32a_currency'}"><c:out value="${item.detail}" /></c:if></c:forEach>" />
                        </div>

                        <div class="form-row" id="wrap_053_of32a_amount" style="display:none;"><span class="label_body">&nbsp;</span><span class="label">Amount</span>
                            <input type="text" id="_053_of32a_amount" name="_053_of32a_amount" maxlength="15" value="<c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_053_of32a_amount'}"><c:out value="${item.detail}" /></c:if></c:forEach>" />
                        </div>
                        <hr/>
                        ...
                        <div class="form-row"><span class="label_body">OF57a</span><span class="label">Account With Bank</span>
                            <select id="_060_of57a_account_with_bank" name="_060_of57a_account_with_bank" onchange="toggle57aOption(this.value)">
                                <option value="">choose a type</option>
                                <option value="A" <c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_060_of57a_account_with_bank'}"><c:if test="${item.detail=='A'}"> selected="true" </c:if></c:if></c:forEach>>Option A (BIC)</option>
                        ...
                        <div class="form-row" id="wrap_063_of57a_name_address" style="display:none;"><span class="label_body">&nbsp;</span><span class="label">Name &amp; Address</span>
                            <textarea id="_063_of57a_name_address" name="_063_of57a_name_address" rows="4" maxlength="140"><c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_063_of57a_name_address'}"><c:out value="${item.detail}" /></c:if></c:forEach></textarea>
                        </div>
                        <hr/>
                        ...
                        <div class="form-row"><span class="label_body">OF72Z</span><span class="label">Sender to Receiver Information</span>
                            <textarea name="_080_of72z_sender_to_receiver_information" id="_080_of72z_sender_to_receiver_information" rows="6" maxlength="210"><c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_080_of72z_sender_to_receiver_information'}"><c:out value="${item.detail}" /></c:if></c:forEach></textarea>
                        </div>
                        <hr/>

                        <!-- OF79Z Narrative -->
                        <div class="form-row"><span class="label_body">OF79Z</span><span class="label">Narrative</span>
                            <textarea name="_090_of79z_narrative" id="_090_of79z_narrative" rows="6" maxlength="1750"><c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_090_of79z_narrative'}"><c:out value="${item.detail}" /></c:if></c:forEach></textarea>
                        </div>
                        <hr/>
