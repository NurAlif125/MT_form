...
@WebServlet("/SCDuplicateCnfList")
public class SCDuplicateCnfList extends HttpServlet {
...
//        int offset = (pageNumber - 1) * length; // (3 - 1) * 10 = 20
        ...
        try {
            log.info("SC");
            int totalRecords = 0;
            duplicate = bBHeaders.getAllHeaderDuplicateCNF(httpSession, channel, start, length, quickSearch, sort);
            totalRecords = bBHeaders.getCountAllHeaderDuplicateCNF(httpSession, channel, quickSearch);
