 ...
 * @author HP
 */
public class SCPrintMXDoc extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        byte[] bytes = null;
        try {
            if (messageType.toLowerCase().contains("pacs.008")){ 
                headers = bBHeaders.printMXDocPacs008(request.getParameter("id"));
            } else if(messageType.toLowerCase().contains("pacs.004")) {
                headers = bBHeaders.printMXDocPacs004(request.getParameter("id"));
            } else {
                headers = bBHeaders.printMXDocPacs009(request.getParameter("id"));
            }
            Map model = new HashMap();
            model.put("username", (String) session.getAttribute("user_id"));
//            model.put("mxdetail", (String) session.getAttribute("user_id"));
