package com.vensys.appcm.controller;

import com.prowidesoftware.swift.model.SwiftBlock4;
import com.prowidesoftware.swift.model.Tag;
import com.vensys.appcm.attribute.AMT940;
import com.vensys.appcm.model.MT940;
import com.google.gson.Gson;  
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;

public class TagMT940 { 
    Logger log = LogManager.getLogger(getClass().getName());
    MT940 mt940 = new MT940();    
    AMT940 atributeMT940 = new AMT940();
    CHeader ch;    
    int count71f = 0;
    public TagMT940(CHeader ch ){
        this.ch = ch;
    }
    public void tagMT940( SwiftBlock4 sb4, int idHeader)  {
        mt940 = atributeMT940.getAtributeMT940();
        log.info("tagMT940: " + new Gson().toJson(mt940));
        log.info("tagMT940: " + sb4.getTags().size());
        String tagValue20 = "";
        for (Tag t : sb4.getTags()) {
            String tagName = t.getName();
            String tagValue = t.getValue();
            String tags = "";
            String tagValue61 = "";
            if (tagName.equalsIgnoreCase("20")) {
                tags = mt940.getMf20() ;
                this.ch.readBlock4(tags, tagName, tagValue);
                tagValue20 = tagValue;
                // System.out.println("tag20: " + tagValue20);
                log.info("tag20: " + tagValue20);
            } else if (tagName.equalsIgnoreCase("21")) {
                tags = mt940.getOf21();
                this.ch.readBlock4(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("25")) {
                tags = mt940.getMf25();
                this.ch.readBlock4(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("28C")) {
                tags = mt940.getMf28c();
                this.ch.readBlock4(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("60F")) {
                tags = mt940.getMf60f();
                this.ch.splitBalance(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("60M")) {
                tags = mt940.getMf60m();
                this.ch.splitBalance(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("61")) {
                tags = mt940.getOf61();
                this.ch.readBlock4(tags, tagName, tagValue);
                tagValue61 = tagValue;
            } else if (tagName.equalsIgnoreCase("86")) {
                tags = mt940.getOf86();
                this.ch.readBlock4(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("62F")) {
                tags = mt940.getMf62f();
                this.ch.splitBalance(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("62M")) {
                tags = mt940.getMf62m();
                this.ch.splitBalance(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("64")) {
                tags = mt940.getOf64();
                this.ch.splitBalance(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("65")) {
                tags = mt940.getOf65();
                this.ch.splitBalance(tags, tagName, tagValue);
            } else if (tagName.equalsIgnoreCase("86")) {
                tags = mt940.getOf86();
                this.ch.readBlock4(tags, tagName, tagValue);
            }
        }      
    }
}
