...
//                System.out.println("Secret berhasil dimuat (panjang): " + secretKey);
            ...
            String encSftpUser = prop.getProperty("sftpUser");
            if (encSftpUser != null && !encSftpUser.trim().isEmpty()) {
                sftpUser = enc.decryptTD(encSftpUser, secretKey);
            } else {
                sftpUser = ""; // atau null
            }
            
            String encPass = prop.getProperty("sftpPassword");
            if (encPass != null && !encPass.trim().isEmpty()) {
                sftpPassword = enc.decryptTD(encPass, secretKey);
            } else {
                sftpPassword = ""; // atau null
            }
