...
//        String tanggal_transaksi = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date());
        ...
        try {
            String sql = "UPDATE headers SET applicationId=?,serviceId=?,logicalTerminal=?,"
                    + "io_type=?,messageType=?,receiverAddress=?,messagePriority=?,"
                    + "deliveryMonitoring=?,obsolescencePeriod=?,bankingPriority=?,komentar=?,"
            ...
            st.setString(4, "I");   //io_type/
            st.setString(5, data.getMessageType());  //messageType/
            st.setString(6, data.getReceiver_institution()); //receiverAddress/

            st.setString(7, data.getPriority());   //messagePriority/
            st.setString(8, data.getMonitoring());   //deliveryMonitoring/
            st.setString(9, "");   //obsolescencePeriod/
            st.setString(10, data.getBanking_priority()); //bankingPriority/
//            st.setString(13, data.getMur()); //mur/

            st.setString(11, data.getOperator_comment());   //komentar/
            st.setString(12, flag);  //flag/
            st.setString(13, user_id);   //userEdit/
...
//            st.setString(17, "");   //templateName//
            st.setInt(14, 0);   //flagTemplate//
            st.setString(15, "");   //senderInputTime//
            st.setString(16, "");   //MIRDate//
            st.setString(17, "");   //MIRLogicalTerminal//

            st.setString(18, "");   //MIRSessionNumber//
            st.setString(19, "");   //MIRSequenceNumber//
            st.setString(20, "");   //receiverOutputDate//
            st.setString(21, "");   //receiverOutputTime//
            st.setString(22, data.getBlock3());   //block3/
            String mergeRefUETR = reference.replace("Reference: ", "") + data.getBlock3().replace("121:", "").replace(";", "");
            st.setString(23, mergeRefUETR);
            st.setInt(24, id_headers);
...
//            System.out.println(st);
        ...
        if ((flag.equalsIgnoreCase("AUTH") && AUTH)
            ...
            if (networkType.contains("MX")) {
                if (!"INC-CVT-RESEND".equalsIgnoreCase(flag) && !"INC-RESEND".equalsIgnoreCase(flag) && "I".equalsIgnoreCase(io_type)) {
...
//                System.out.println(finalMX.get("final_mx"));
                    ...
                    log.info("STL MX for id_headers " + id_headers);
                    Map<String, String> finalMX = getMxTextById(id_headers.toString());
                    AbstractMX abstractMX = AbstractMX.parse(finalMX.get("final_mx"));
                    var variant = abstractMX.getMxId().id();
                    String source = getSource(id_headers);
                    ct.createTextFileMX(source, finalMX.get("final_mx"), variant, id_headers, "O", source, flag, user_id, ip_access, comp_name);
                }
            } else {
...
//        int id = id_headers();
        try {
            ...
            String sql = "INSERT INTO tags (urutan,id_headers,tag,detail,tagName,info)"
            + "SELECT ?, ?, ?, ?, ?, ?"
            ...
            st.setInt(8, id);
            st.setString(9, tag);
            st.setString(10, detail);
            st.setString(11, tagName);
            st.setString(12, "");
...
//            System.out.println("QUERNYAAAAAAAAA: "+st.toString());
            st.executeUpdate();
        } catch (SQLException e) {
//            e.printStackTrace();
...
//20190923 ditambah or karena ada 59f
    ...
    public String getType71B(int id) { // ditambahkan 250807
        String type = "";
        try {
            String sql = "Select detail from tags where tag ='71B' and id_headers=" + id;
            PreparedStatement st = this.conn.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                type = rs.getString(1);
