...
//        }
        ...
        while (rs.next()) {
            Header header = new Header();
            header.setMessageType(rs.getString(2));
            String logTer = (rs.getString(3) != null && rs.getString(3).length() > 9) 
                ? rs.getString(3).substring(0, 8) + rs.getString(3).substring(9)
                : "";

String recAdd = (rs.getString(7) != null && rs.getString(7).length() > 9) 
                ? rs.getString(7).substring(0, 8) + rs.getString(7).substring(9)
                : "";
...
//        System.out.println("sql=" + sql);
        ...
        while (rs.next()) {
            Header header = new Header();
            String logTer = (rs.getString(1) != null && rs.getString(1).length() > 9) 
                ? rs.getString(1).substring(0, 8) + rs.getString(1).substring(9)
                : "";

            String recAdd = (rs.getString(3) != null && rs.getString(3).length() > 9) 
                            ? rs.getString(3).substring(0, 8) + rs.getString(3).substring(9)
                            : "";
    ...
    public Collection<Header> printMTNota(String id) throws SQLException {
        ...
        while (rs.next()) {
            ...
            String logTer = (rs.getString(2) != null && rs.getString(2).length() > 9) 
                ? rs.getString(2).substring(0, 8) + rs.getString(2).substring(9)
                : "";

            String recAdd = (rs.getString(4) != null && rs.getString(4).length() > 9) 
                            ? rs.getString(4).substring(0, 8) + rs.getString(4).substring(9)
                            : "";
...
//        TYPE:INCOMING,TYPE:OUTGOING
        ...
        if (list.contains("TYPE:OUTGOING") || list.contains("TYPE:INCOMING")) {
...
//        SRC:MANUAL,SRC:FIA
        if ("Treasury OPS".equalsIgnoreCase(channel)) {
            where += "userEntry IN ('SRC:MANUAL', 'SRC:FIA') AND LEFT(h.logicalTerminal, 7) || RIGHT(h.logicalTerminal, 3) IN ('BDINIDJTRS','BDINIDJXXX')";
...
//            where += " AND flag='TEXT' AND CAST(h.tanggal as date) = '" + tanggal_transaksi_sebulan + "'";
        ...
        if (channel != null && !channel.isBlank()) {
            if (channel.equalsIgnoreCase("EMS")) {
                ...
                where += " AND source IN ('FRONTARENA', 'TSA', 'Treasury OPS')";
            } else {
                where += " AND source LIKE '%" + channel + "%'";
            }
        }
        
//        List<Map<String, Object>> lists = (List<Map<String, Object>>) httpSession.getAttribute("tieringLimit");
//        if (lists != null && !lists.isEmpty()) {
//            String limit = " AND (";
//            for (Map<String, Object> item : lists) {
//                limit += "(td.trans_ccy = '" + item.get("currency") + "'"+
//                " AND td.trans_amount BETWEEN " + item.get("min_limit") +
//                " AND " + item.get("max_limit") + " AND h.userEntry='SRC:MANUAL') OR ";
//            }
//            if (limit.length() > 6) { 
//                limit = limit.substring(0, limit.length() - 3);
//                limit += ") ";
//                where += limit;
//            }
//        } else {
//            System.out.println("Attribute 'tieringLimit' empty!");
//        }
            ...
            if (lists != null && !lists.isEmpty()) {
                ...
                // tambahkan kondisi SRC:FIA
                limit += " OR h.userEntry='SRC:FIA') ";                                                                                                      
                where += limit;
            } else {
                int grpLimit = Integer.parseInt((String) httpSession.getAttribute("group_limit")); 
                if(grpLimit < 0) {
                    String limit = " AND ((td.trans_ccy='' AND td.trans_amount BETWEEN 0 and 0 AND h.userEntry='SRC:MANUAL') OR h.userEntry='SRC:FIA')";
                    where += limit;
                }
                System.out.println("Attribute 'tieringLimit' empty!");
            }
        ...
        if (criteria.getSourceSearch() != null && !criteria.getSourceSearch().isEmpty()) {
            ...
            if (source.startsWith("tre") || source.startsWith("Tre") || source.startsWith("TRE")) {
                where += " AND (source IN ('FRONTARENA', 'TSA') OR LEFT(h.logicalTerminal, 7) || RIGHT(h.logicalTerminal, 3) IN ('BDINIDJTRS','BDINIDJXXX'))";
            } else if (source.startsWith("tra") || source.startsWith("Tra") || source.startsWith("TRA")) {
                where += " AND (source = 'BANKTRADE' OR LEFT(logicalTerminal, 7) || RIGHT(logicalTerminal, 3) = 'BDINIDJCLC')";
            } else if (source.startsWith("c") || source.startsWith("C")) {
                where += " AND (source = 'CSA' OR source = 'CUSTODY' OR LEFT(logicalTerminal, 7) || RIGHT(logicalTerminal, 3) = 'BDINIDJCUS')";
            } else if (source.startsWith("r") || source.startsWith("R")) {
                where += " AND (source = 'NCBS' OR LEFT(logicalTerminal, 7) || RIGHT(logicalTerminal, 3) = 'BDINIDJRMT')";
            } else {
...
//        System.out.println(sql);
        ...
        while (rs.next()) {
            ...
            //header.setReceiverAddress( // ditambahkan 14102025
            //    rs.getString(7) == null ? null : rs.getString(7).toUpperCase()
            //);
            //header.setReceiverAddress(rs.getString(7).toUpperCase()); dibawah ini update yg barunya
...
//        System.out.println("sql retur=" + sql);
    ...
    public List<Header> getAllHeaderDuplicate(HttpSession httpSession, String channel, int start, int length, String quickSearch, String sort) throws Exception {
        Date tanggal = new Date();
        SimpleDateFormat dDay = new SimpleDateFormat("yyyy-MM-dd");
...
//        }
         ...
         if (channel != null && !channel.isBlank()) {
            if (channel.equalsIgnoreCase("EMS")) {
                where += " AND h.source IN ('EMS', 'NCBS') ";
            } else if (channel.equalsIgnoreCase("CUSTODY") || "CSA".equalsIgnoreCase(channel)) {
                where += " AND source IN ('CSA', 'CUSTODY') ";
            } else if(channel.equalsIgnoreCase("Treasury OPS")) {
                where += " AND source IN ('FRONTARENA', 'TSA', 'Treasury OPS') ";
            } else {
                where += " AND h.source ILIKE ? ";
                params.add("%" + channel + "%");
            }
        }
        
        if ("Treasury OPS".equalsIgnoreCase(channel)) {
            where += " AND h.userEntry IN ('SRC:MANUAL', 'SRC:FIA') AND LEFT(h.logicalTerminal, 7) || RIGHT(h.logicalTerminal, 3) IN ('BDINIDJTRS','BDINIDJXXX') ";
        }
    ...
    public int getCountAllHeaderDuplicate(HttpSession httpSession, String channel, String quickSearch) throws Exception {
        Date tanggal = new Date();
        SimpleDateFormat dDay = new SimpleDateFormat("yyyy-MM-dd");
        List<Header> headers = new ArrayList<>();
...
//        }
    ...
    public List<Header> getAllHeaderDuplicateCNF(HttpSession httpSession, String channel, int start, int length, String quickSearch, String sort) throws Exception {
        Date tanggal = new Date();
        SimpleDateFormat dDay = new SimpleDateFormat("yyyy-MM-dd");
        List<Header> headers = new ArrayList<>();
...
//            }
//            debugQuery = debugQuery.replaceFirst("\\?", value); // ganti satu per satu
//        }
//
//        //  Print query dengan parameter
//        System.out.println("Debug SQL: " + debugQuery);
    ...
    public int getCountAllHeaderDuplicateCNF(HttpSession httpSession, String channel, String quickSearch) throws Exception {
        Date tanggal = new Date();
        SimpleDateFormat dDay = new SimpleDateFormat("yyyy-MM-dd");
        List<Header> headers = new ArrayList<>();
...
//        }
        ...
        if (channel != null && !channel.isBlank()) {
            if (channel.equalsIgnoreCase("EMS")) {
                ...
                where.append(" AND source IN ('FRONTARENA', 'TSA', 'Treasury OPS')");
            } else {
        ...
        if ("Treasury OPS".equalsIgnoreCase(channel)) {
            where.append(" AND h.userEntry IN ('SRC:MANUAL', 'SRC:FIA') AND LEFT(h.logicalTerminal, 7) || RIGHT(h.logicalTerminal, 3) IN ('BDINIDJTRS','BDINIDJXXX')");
        }
...
//                parameters.add("%" + criteria.getIoSearch() + "%");
            ...
            if (criteria.getSourceSearch() != null && !criteria.getSourceSearch().isEmpty()) {
                String source = criteria.getSourceSearch().trim();
                if (source.startsWith("tre") || source.startsWith("Tre") || source.startsWith("TRE")) {
                    where.append(" AND (source IN ('FRONTARENA', 'TSA') OR LEFT(h.logicalTerminal, 7) || RIGHT(h.logicalTerminal, 3) IN ('BDINIDJTRS','BDINIDJXXX'))");
                } else if (source.startsWith("tra") || source.startsWith("Tra") || source.startsWith("TRA")) {
                    where.append(" AND (source = 'BANKTRADE' OR LEFT(logicalTerminal, 7) || RIGHT(logicalTerminal, 3) = 'BDINIDJCLC')");
                } else if (source.startsWith("c") || source.startsWith("C")) {
                    where.append(" AND (source = 'CSA' OR source = 'CUSTODY' OR LEFT(logicalTerminal, 7) || RIGHT(logicalTerminal, 3) = 'BDINIDJCUS')");
                } else if (source.startsWith("r") || source.startsWith("R")) {
                    where.append(" AND (source = 'NCBS' OR LEFT(logicalTerminal, 7) || RIGHT(logicalTerminal, 3) = 'BDINIDJRMT')");
                } else {
                    where.append(" AND h.source ILIKE ?");
...
//                + "WHERE h.id_headers='" + headerId + "'";
        ...
        while (rs.next()) {
            ...
            header.setChannel(rs.getString("source"));
        }
        return header;
    }
...
//        System.out.println("sql=" + sql);
        ...
        while (rs.next()) {
            ...
            if ("721".equalsIgnoreCase(rs.getString(2)) || "722".equalsIgnoreCase(rs.getString(2))) {
                tag.setTagName(rs.getString(2).substring(0, 2).toUpperCase());
            } else if ("52A1".equalsIgnoreCase(rs.getString(2)) || "52A2".equalsIgnoreCase(rs.getString(2)) || "52D1".equalsIgnoreCase(rs.getString(2)) || "52D2".equalsIgnoreCase(rs.getString(2))
                    || "56A1".equalsIgnoreCase(rs.getString(2)) || "56A2".equalsIgnoreCase(rs.getString(2)) || "57A1".equalsIgnoreCase(rs.getString(2)) || "57A2".equalsIgnoreCase(rs.getString(2))
                    || "57B1".equalsIgnoreCase(rs.getString(2)) || "57B2".equalsIgnoreCase(rs.getString(2)) || "57D1".equalsIgnoreCase(rs.getString(2)) || "57D2".equalsIgnoreCase(rs.getString(2))
                    || "56D1".equalsIgnoreCase(rs.getString(2)) || "56D2".equalsIgnoreCase(rs.getString(2))) {
                tag.setTagName(rs.getString(2).substring(0, 3).toUpperCase());
            } else {
...
//            System.out.println(i + " = data = " + arr[i]);
    ...
    public List<TagDB> getAllTagById(String headerId, String prefix) throws Exception {
        ...
        String sql = "SELECT DISTINCT urutan,tag,detail,tagName,info FROM " + prefix + "tags WHERE id_headers='" + headerId + "'";
//        System.out.println("sql=" + sql);
        PreparedStatement st = this.conn.prepareStatement(sql);
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
...
//        NumberFormat nf2 = NumberFormat.getInstance(new Locale("us", "US")); //hasilnya pake koma(,)
    ...
    public EssentialFieldPacs008 getEssentialFieldPacs008byId(int id_headers) {
        ...
        try {
            ...
            LogManager.getLogger(DBHeader.class.getName()).log(Level.ERROR, ex);
        }
        return null;
    }
...
//            dataEssentialFieldPacs009.setPreviousInstructingAgent3(resultSet.getString("previousInstructingAgent3"));
    ...
    public Collection<Header> printMXDocPacs008(String id) throws SQLException, Exception {
        ...
        while (rs.next()) {
            Header header = new Header();
            String logTer = (rs.getString(1) != null && rs.getString(1).length() >= 8)
                    ? rs.getString(1).substring(0, 8) + "XXX"
                    : "";

            String recAdd = (rs.getString(3) != null && rs.getString(3).length() >= 8)
                    ? rs.getString(3).substring(0, 8) + "XXX"
                    : "";
            
            
            String col13 = (rs.getString(13) != null) ? rs.getString(13) : "";
            if (col13.equalsIgnoreCase("O")) {
                header.setLogicalTerminal(logTer);
                header.setReceiverAddress(recAdd);
            } else {
                header.setLogicalTerminal(recAdd);
            ...
            header.setMessageType((rs.getString("messageType") != null) ? rs.getString("messageType") : "");
            header.setMessagePriority((rs.getString(4) != null) ? rs.getString(4) : "");
            header.setBankingPriority((rs.getString(5) != null) ? rs.getString(5) : "");
            header.setMur((rs.getString(6) != null) ? rs.getString(6) : "");
            header.setKomentar((rs.getString(7) != null) ? rs.getString(7) : "");
            header.setMIRDate((rs.getString(8) != null) ? rs.getString(8).trim() : "");

            String mirLogicalTerm = (rs.getString(3) != null && rs.getString(3).length() >= 8)
                    ? rs.getString(3).substring(0, 8) + "XXX"
                    : "";
            header.setMIRLogicalTerminal(mirLogicalTerm);

            header.setMIRSessionNumber((rs.getString(10) != null) ? rs.getString(10) : "");
            header.setMIRSequenceNumber((rs.getString(11) != null) ? rs.getString(11) : "");
            header.setFlag((rs.getString(15) != null) ? rs.getString(15) : "");

            String data = "";
            data += "Requestor DN\t\t\t: ou=" 
                    + ((rs.getString(12) != null && rs.getString(12).length() > 8) ? rs.getString(12).substring(8).toLowerCase() : "")
                    + ",o=" + ((rs.getString(1) != null && rs.getString(1).length() >= 8) ? rs.getString(1).substring(0, 8).toLowerCase() : "")
                    + ",o=swift\n";

            data += "Responder DN\t\t\t: ou=" 
                    + ((rs.getString(13) != null && rs.getString(13).length() > 8) ? rs.getString(13).substring(8).toLowerCase() : "")
                    + ",o=" + ((rs.getString(3) != null && rs.getString(3).length() >= 8) ? rs.getString(3).substring(0, 8).toLowerCase() : "")
                    + ",o=swift\n";

            data += "Identifier\t\t\t\t: " + ((rs.getString(14) != null) ? rs.getString(14) : "") + "\n";
            data += "Message Identifier\t\t: " + ((rs.getString(16) != null) ? rs.getString(16) : "") + "\n";
            data += "Instruction Id\t\t\t: " + ((rs.getString(67) != null) ? rs.getString(67) : "") + "\n";
            data += "Interbank Settlement Date\t: " + ((rs.getString(17) != null) ? rs.getString(17) : "") + "\n";
            data += "Currency\t\t\t: " + ((rs.getString(18) != null) ? rs.getString(18) : "") + "\n";

            data += "Interbank Settlement Amount\t: "
                    + ((rs.getString(19) != null && !rs.getString(19).isEmpty()) 
                        ? df.format(new BigDecimal(rs.getString(19))) 
                        : "")
                    + "\n";

            data += "Charges\t\t\t\t: " + ((rs.getString(20) != null) ? rs.getString(20) : "") + "\n";
...
//    public Collection<Header> printMXDocPacs004(String id) throws SQLException, Exception {
//        ArrayList<Header> headers = new ArrayList<Header>();
//        DecimalFormat df = new DecimalFormat("#,###.####");
//        String sql = "SELECT logicalTerminal,messageType,receiverAddress,messagePriority,\n"
...
//                + "json_tag->'appHdr'->>'msgDefIdr' AS msgDefIdr,\n"
//                + "h.flag,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->>'rtrId' AS instructionId,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->>'orgnlEndToEndId' AS endToEndId,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->>'orgnlUetr' AS uetr,\n"
//                + "CONCAT(mx.json_tag->'pmtRtr'->'txInf'->0->'intrBkSttlmDt'->>'year', '-',\n"
//                + "LPAD(mx.json_tag->'pmtRtr'->'txInf'->0->'intrBkSttlmDt'->>'month', 2, '0'), '-',\n"
//                + "LPAD(mx.json_tag->'pmtRtr'->'txInf'->0->'intrBkSttlmDt'->>'day', 2, '0')) AS interbankSettlementDate,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrdIntrBkSttlmAmt'->>'ccy' AS interBankSettlementCcy,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrdIntrBkSttlmAmt'->>'value' AS interBankSettlementAmount,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'agt'->'finInstnId'->>'bicfi' AS debtorInstitutionId,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->>'nm' as debtorName,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'dept' as debtorDept,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'subDept' as debtorSubDept,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'strtNm' as debtorStrtNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'bldgNb' as debtorBldgNb,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'bldgNm' as debtorBldgNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'flr' as debtorFlr,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'pstBx' as debtorPstBx,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'room' as debtorRoom,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'pstCd' as debtorPstCd,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'twnNm' as debtorTwnNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'twnLctnNm' as debtorTwnLctnNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'dstrctNm' as debtorDstrctNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->>'ctry' as debtorCtry,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->'adrLine'->>0 as debtorAdrLine0,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->'adrLine'->>1 as debtorAdrLine1,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtr'->'pty'->'pstlAdr'->'adrLine'->>2 as debtorAdrLine2,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'agt'->'finInstnId'->>'bicfi' AS creditorInstitutionId,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->>'nm' as creditorName,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'dept' as creditorDept,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'subDept' as creditorSubDept,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'strtNm' as creditorStrtNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'bldgNb' as creditorBldgNb,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'bldgNm' as creditorBldgNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'flr' as creditorFlr,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'pstBx' as creditorPstBx,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'room' as creditorRoom,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'pstCd' as creditorPstCd,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'twnNm' as creditorTwnNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'twnLctnNm' as creditorTwnLctnNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'dstrctNm' as creditorDstrctNm,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->>'ctry' as creditorCtry,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->'adrLine'->>0 as creditorAdrLine0,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->'adrLine'->>1 as creditorAdrLine1,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtr'->'pty'->'pstlAdr'->'adrLine'->>2 as creditorAdrLine2,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'instgAgt'->'finInstnId'->>'bicfi' AS instructingAgent,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'instdAgt'->'finInstnId'->>'bicfi' AS instructedAgent,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'dbtrAgt'->'finInstnId'->>'bicfi' AS debtorAgent,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrChain'->'cdtrAgt'->'finInstnId'->>'bicfi' AS creditorAgent,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrRsnInf'->0->'addtlInf'->>0 as returnReasonInf,\n"
//                + "mx.json_tag->'pmtRtr'->'txInf'->0->'rtrRsnInf'->0->'rsn'->>'cd' as returnReasonCd,\n"
//                + "h.io_type as io_type "
//                + "from headers h left join tags_mx mx on h.id_headers=mx.id_headers where h.id_headers=?";
//        PreparedStatement st = this.conn.prepareStatement(sql);
//        st.setInt(1, Integer.parseInt(id));
//        ResultSet rs = st.executeQuery();
//
//        while (rs.next()) {
//            Header header = new Header();
//            String logTer = (rs.getString("logicalTerminal") != null && rs.getString(1).length() >= 8)
//                    ? rs.getString(1).substring(0, 8) + "XXX"
//                    : "";
//
//            String recAdd = (rs.getString("receiverAddress") != null && rs.getString(3).length() >= 8)
//                    ? rs.getString(3).substring(0, 8) + "XXX"
//                    : "";
//
...
//            data += "Requestor DN\t\t\t\t: ou=" 
//                    + ((rs.getString("fromBic") != null && rs.getString("fromBic").length() > 8) 
//                        ? rs.getString("fromBic").substring(8).toLowerCase() 
//                        : "")
//                    + ",o=" 
//                    + ((rs.getString(1) != null && rs.getString(1).length() >= 8) 
//                        ? rs.getString(1).substring(0, 8).toLowerCase() 
//                        : "")
//                    + ",o=swift\n";
//
//            data += "Responder DN\t\t\t\t: ou=" 
//                    + ((rs.getString("toBic") != null && rs.getString("toBic").length() > 8) 
//                        ? rs.getString("toBic").substring(8).toLowerCase() 
//                        : "")
//                    + ",o=" 
//                    + ((rs.getString(3) != null && rs.getString(3).length() >= 8) 
//                        ? rs.getString(3).substring(0, 8).toLowerCase() 
//                        : "")
//                    + ",o=swift\n";
//
//
...
//    }
    ...
    public Collection<Header> printMXDocPacs004(String id) throws SQLException, Exception {
        ...
        String sql = "SELECT logicalTerminal,messageType,receiverAddress,messagePriority,\n" +
                ...
                "json_tag->'appHdr'->>'msgDefIdr' AS msgDefIdr,\n" +
                "h.flag, h.io_type,\n" +
                "mx.json_tag->'pmtRtr'->'grpHdr'->>'msgId' AS msgId,\n" +
                "mx.json_tag->'pmtRtr'->'txInf'->0->>'rtrId' AS rtrId,\n" +
                "mx.json_tag->'pmtRtr'->'txInf'->0->>'orgnlEndToEndId' AS endToEndId,\n" +
                "mx.json_tag->'pmtRtr'->'txInf'->0->>'orgnlUetr' AS uetr,\n" +
                "CONCAT(mx.json_tag->'pmtRtr'->'txInf'->0->'intrBkSttlmDt'->>'year', '-',\n" +
                "LPAD(mx.json_tag->'pmtRtr'->'txInf'->0->'intrBkSttlmDt'->>'month', 2, '0'), '-',\n" +
        ...
        while (rs.next()) {
            Header header = new Header();
            String logTer = (rs.getString("logicalTerminal") != null && rs.getString("logicalTerminal").length() >= 8)
                    ? rs.getString("logicalTerminal").substring(0, 8) + "XXX"
                    : "";

            String recAdd = (rs.getString("receiverAddress") != null && rs.getString("receiverAddress").length() >= 8)
                    ? rs.getString("receiverAddress").substring(0, 8) + "XXX"
                    : "";
            
            
            String io = (rs.getString("io_type") != null) ? rs.getString("io_type") : "";
            ...
            header.setMessageType((rs.getString("messageType") != null) ? rs.getString("messageType") : "");
            header.setMessagePriority((rs.getString("messagePriority") != null) ? rs.getString("messagePriority") : "");
            header.setBankingPriority((rs.getString("bankingPriority") != null) ? rs.getString("bankingPriority") : "");
            header.setMur((rs.getString("mur") != null) ? rs.getString("mur") : "");
            header.setKomentar((rs.getString("komentar") != null) ? rs.getString("komentar") : "");
            header.setMIRDate((rs.getString("MIRDate") != null) ? rs.getString("MIRDate").trim() : "");

            String mirLogicalTerm = (rs.getString("receiverAddress") != null && rs.getString("receiverAddress").length() >= 8)
                    ? rs.getString("receiverAddress").substring(0, 8) + "XXX"
                    : "";
            header.setMIRLogicalTerminal(mirLogicalTerm);

            header.setMIRSessionNumber((rs.getString("MIRSessionNumber") != null) ? rs.getString("MIRSessionNumber") : "");
            header.setMIRSequenceNumber((rs.getString("MIRSequenceNumber") != null) ? rs.getString("MIRSequenceNumber") : "");
            header.setFlag((rs.getString("flag") != null) ? rs.getString("flag") : "");

            String data = "";
            data += "Requestor DN\t\t\t: ou=" 
                    + ((rs.getString("fromBic") != null && rs.getString("fromBic").length() > 8) ? rs.getString("fromBic").substring(8).toLowerCase() : "")
                    + ",o=" + ((rs.getString("logicalTerminal") != null && rs.getString("logicalTerminal").length() >= 8) ? rs.getString("logicalTerminal").substring(0, 8).toLowerCase() : "")
                    + ",o=swift\n";

            data += "Responder DN\t\t\t: ou=" 
                    + ((rs.getString("toBic") != null && rs.getString("toBic").length() > 8) ? rs.getString("toBic").substring(8).toLowerCase() : "")
                    + ",o=" + ((rs.getString("receiverAddress") != null && rs.getString("receiverAddress").length() >= 8) ? rs.getString("receiverAddress").substring(0, 8).toLowerCase() : "")
                    + ",o=swift\n";

            data += "Identifier\t\t\t: " + ((rs.getString("msgDefIdr") != null) ? rs.getString("msgDefIdr") : "") + "\n";
            data += "Message Identifier\t\t: " + ((rs.getString("msgId") != null) ? rs.getString("msgId") : "") + "\n";
            data += "Return Id\t\t\t: " + ((rs.getString("rtrId") != null) ? rs.getString("rtrId") : "") + "\n";
            data += "Interbank Settlement Date\t: " + ((rs.getString("interbankSettlementDate") != null) ? rs.getString("interbankSettlementDate") : "") + "\n";
            data += "Currency\t\t\t: " + ((rs.getString("interBankSettlementCcy") != null) ? rs.getString("interBankSettlementCcy") : "") + "\n";

            data += "Interbank Settlement Amount\t: "
                    + ((rs.getString("interBankSettlementAmount") != null && !rs.getString("interBankSettlementAmount").isEmpty()) 
                        ? df.format(new BigDecimal(rs.getString("interBankSettlementAmount"))) 
                        : "")
                    + "\n";
            
            String dbtrInstnId = rs.getString("debtorInstitutionId");
            if (dbtrInstnId == null) {
                dbtrInstnId = "";
            } else {
                dbtrInstnId = "Debtor Institution Id\t\t: " + dbtrInstnId + "\n"; 
            }
            data += dbtrInstnId;
            
            String dbtrNm = rs.getString("debtorName");
            if (dbtrNm == null) {
                dbtrNm = "";
            } else {
                dbtrNm = "Debtor Name\t\t\t: " + dbtrNm + "\n";
            }
            data += dbtrNm;
            
            String dbtrDept = rs.getString("debtorDept");
            if (dbtrDept == null) {
                dbtrDept = "";
            } else {
                dbtrDept = "Debtor Dept\t\t\t: " + dbtrDept + "\n";
            }
            data += dbtrDept;
            
            String dbtrSubDept = rs.getString("debtorSubDept");
            if (dbtrSubDept == null) {
                dbtrSubDept = "";
            } else {
                dbtrSubDept = "Debtor SubDept\t\t\t: " + dbtrSubDept + "\n";
            }
            data += dbtrSubDept;
            
            String dbtrStrtNm = rs.getString("debtorStrtNm");
            if (dbtrStrtNm == null) {
                dbtrStrtNm = "";
            } else {
                dbtrStrtNm = "Debtor Street Name\t\t: " + dbtrStrtNm + "\n";
            }
            data += dbtrStrtNm;
            
            String dbtrBldgNb = rs.getString("debtorBldgNb");
            if (dbtrBldgNb == null) {
                dbtrBldgNb = "";
            } else {
                dbtrBldgNb = "Debtor Building Number\t\t: " + dbtrBldgNb + "\n";
            }
            data += dbtrBldgNb;
            
            String dbtrBldgNm = rs.getString("debtorBldgNm");
            if (dbtrBldgNm == null) {
                dbtrBldgNm = "";
            } else {
                dbtrBldgNm = "Debtor Building Name\t\t: " + dbtrBldgNm + "\n";
            }
            data += dbtrBldgNm;
            
            String dbtrFlr = rs.getString("debtorFlr");
            if (dbtrFlr == null) {
                dbtrFlr = "";
            } else {
                dbtrFlr = "Debtor Floor\t\t\t: " + dbtrFlr + "\n";
            }
            data += dbtrFlr;
            
            String dbtrPstBx = rs.getString("debtorPstBx");
            if (dbtrPstBx == null) {
                dbtrPstBx = "";
            } else {
                dbtrPstBx = "Debtor PostalBox\t\t: " + dbtrPstBx + "\n";
            }
            data += dbtrPstBx;
            
            String dbtrRoom = rs.getString("debtorRoom");
            if (dbtrRoom == null) {
                dbtrRoom = "";
            } else {
                dbtrRoom = "Debtor Room\t\t\t: " + dbtrRoom + "\n";
            }
            data += dbtrRoom;
            
            String dbtrPstCd = rs.getString("debtorPstCd");
            if (dbtrPstCd == null) {
                dbtrPstCd = "";
            } else {
                dbtrPstCd = "Debtor PostalCode\t\t: " + dbtrPstCd + "\n";
            }
            data += dbtrPstCd;
            
            String dbtrTwnNm = rs.getString("debtorTwnNm");
            if (dbtrTwnNm == null) {
                dbtrTwnNm = "";
            } else {
                dbtrTwnNm = "Debtor Town Name\t\t: " + dbtrTwnNm + "\n";
            }
            data += dbtrTwnNm;
            
            String dbtrTwnLctnNm = rs.getString("debtorTwnLctnNm");
            if (dbtrTwnLctnNm == null) {
                dbtrTwnLctnNm = "";
            } else {
                dbtrTwnLctnNm = "Debtor Town Location Name\t: " + dbtrTwnLctnNm + "\n";
            }
            data += dbtrTwnLctnNm;
            
            String dbtrDstrctNm = rs.getString("debtorDstrctNm");
            if (dbtrDstrctNm == null) {
                dbtrDstrctNm = "";
            } else {
                dbtrDstrctNm = "Debtor District Name\t\t: " + dbtrDstrctNm + "\n";
            }
            data += dbtrDstrctNm;
            
            String dbtrCtry = rs.getString("debtorCtry");
            if (dbtrCtry == null) {
                dbtrCtry = "";
            } else {
                dbtrCtry = "Debtor Country\t\t\t: " + dbtrCtry + "\n";
            }
            data += dbtrCtry;
            
            String dbtrAdr0 = rs.getString("debtorAdrLine0");
            if (dbtrAdr0 == null) {
                dbtrAdr0 = "";
            } else {
                dbtrAdr0 = "Debtor AddressLine\t\t: " + dbtrAdr0 + "\n";
            }
            data += dbtrAdr0;
            
            String dbtrAdr1 = rs.getString("debtorAdrLine1");
            if (dbtrAdr1 == null) {
                dbtrAdr1 = "";
            } else {
                dbtrAdr1 = "Debtor AddressLine\t\t: " + dbtrAdr1 + "\n";
            }
            data += dbtrAdr1;
            
            String dbtrAdr2 = rs.getString("debtorAdrLine2");
            if (dbtrAdr2 == null) {
                dbtrAdr2 = "";
            } else {
                dbtrAdr2 = "Debtor AddressLine\t\t: " + dbtrAdr2 + "\n";
            } 
            data += dbtrAdr2;
            
            String cdtrInstnId = rs.getString("creditorInstitutionId");
            if (cdtrInstnId == null) {
                cdtrInstnId = "";
            } else {
                cdtrInstnId = "Creditor Institution Id\t\t: " + cdtrInstnId + "\n"; 
            }
            data += cdtrInstnId;
            
            String cdtrNm = rs.getString("creditorName");
            if (cdtrNm == null) {
                cdtrNm = "";
            } else {
                cdtrNm = "Creditor Name\t\t\t: " + cdtrNm + "\n";
            }
            data += cdtrNm;
            
            String cdtrDept = rs.getString("creditorDept");
            if (cdtrDept == null) {
                cdtrDept = "";
            } else {
                cdtrDept = "Creditor Dept\t\t\t: " + cdtrDept + "\n";
            }
            data += cdtrDept;
            
            String cdtrSubDept = rs.getString("creditorSubDept");
            if (cdtrSubDept == null) {
                cdtrSubDept = "";
            } else {
                cdtrSubDept = "Creditor SubDept\t\t\t: " + cdtrSubDept + "\n";
            }
            data += cdtrSubDept;
            
            String cdtrStrtNm = rs.getString("creditorStrtNm");
            if (cdtrStrtNm == null) {
                cdtrStrtNm = "";
            } else {
                cdtrStrtNm = "Creditor Street Name\t\t: " + cdtrStrtNm + "\n";
            }
            data += cdtrStrtNm;
            
            String cdtrBldgNb = rs.getString("creditorBldgNb");
            if (cdtrBldgNb == null) {
                cdtrBldgNb = "";
            } else {
                cdtrBldgNb = "Creditor Building Number\t\t: " + cdtrBldgNb + "\n";
            }
            data += cdtrBldgNb;
            
            String cdtrBldgNm = rs.getString("creditorBldgNm");
            if (cdtrBldgNm == null) {
                cdtrBldgNm = "";
            } else {
                cdtrBldgNm = "Creditor Building Name\t\t: " + cdtrBldgNm + "\n";
            }
            data += cdtrBldgNm;
            
            String cdtrFlr = rs.getString("creditorFlr");
            if (cdtrFlr == null) {
                cdtrFlr = "";
            } else {
                cdtrFlr = "Creditor Floor\t\t\t: " + cdtrFlr + "\n";
            }
            data += cdtrFlr;
            
            String cdtrPstBx = rs.getString("creditorPstBx");
            if (cdtrPstBx == null) {
                cdtrPstBx = "";
            } else {
                cdtrPstBx = "Creditor PostalBox\t\t: " + cdtrPstBx + "\n";
            }
            data += cdtrPstBx;
            
            String cdtrRoom = rs.getString("creditorRoom");
            if (cdtrRoom == null) {
                cdtrRoom = "";
            } else {
                cdtrRoom = "Creditor Room\t\t\t: " + cdtrRoom + "\n";
            }
            data += cdtrRoom;
            
            String cdtrPstCd = rs.getString("creditorPstCd");
            if (cdtrPstCd == null) {
                cdtrPstCd = "";
            } else {
                cdtrPstCd = "Creditor PostalCode\t\t: " + cdtrPstCd + "\n";
            }
            data += cdtrPstCd;
            
            String cdtrTwnNm = rs.getString("creditorTwnNm");
            if (cdtrTwnNm == null) {
                cdtrTwnNm = "";
            } else {
                cdtrTwnNm = "Creditor Town Name\t\t: " + cdtrTwnNm + "\n";
            }
            data += cdtrTwnNm;
            
            String cdtrTwnLctnNm = rs.getString("creditorTwnLctnNm");
            if (cdtrTwnLctnNm == null) {
                cdtrTwnLctnNm = "";
            } else {
                cdtrTwnLctnNm = "Creditor Town Location Name\t: " + cdtrTwnLctnNm + "\n";
            }
            data += cdtrTwnLctnNm;
            
            String cdtrDstrctNm = rs.getString("creditorDstrctNm");
            if (cdtrDstrctNm == null) {
                cdtrDstrctNm = "";
            } else {
                cdtrDstrctNm = "Creditor District Name\t\t: " + cdtrDstrctNm + "\n";
            }
            data += cdtrDstrctNm;
            
            String cdtrCtry = rs.getString("creditorCtry");
            if (cdtrCtry == null) {
                cdtrCtry = "";
            } else {
                cdtrCtry = "Creditor Country\t\t\t: " + cdtrCtry + "\n";
            }
            data += cdtrCtry;
            
            String cdtrAdr0 = rs.getString("creditorAdrLine0");
            if (cdtrAdr0 == null) {
                cdtrAdr0 = "";
            } else {
                cdtrAdr0 = "Creditor AddressLine\t\t: " + cdtrAdr0 + "\n";
            }
            data += cdtrAdr0;
            
            String cdtrAdr1 = rs.getString("creditorAdrLine1");
            if (cdtrAdr1 == null) {
                cdtrAdr1 = "";
            } else {
                cdtrAdr1 = "Creditor AddressLine\t\t: " + cdtrAdr1 + "\n";
            }
            data += cdtrAdr1;
            
            String cdtrAdr2 = rs.getString("creditorAdrLine2");
            if (cdtrAdr2 == null) {
                cdtrAdr2 = "";
            } else {
                cdtrAdr2 = "Creditor AddressLine\t\t: " + cdtrAdr2 + "\n";
            } 
            data += cdtrAdr2;
            
            String instgAgt = rs.getString("instructingAgent");
            if (instgAgt == null) {
                instgAgt = "";
            } else {
                instgAgt = "Instructing Agent\t\t: " + instgAgt + "\n";
            }
            data += instgAgt;
            
            String instdAgt = rs.getString("instructedAgent");
            if (instdAgt == null) {
                instdAgt = "";
            } else {
                instdAgt = "Instructed Agent\t\t: " + instdAgt + "\n";
            }
            data += instdAgt;
            
            String dbtrAgt = rs.getString("debtorAgent");
            if (dbtrAgt == null) {
                dbtrAgt = "";
            } else {
                dbtrAgt = "Debtor Agent\t\t\t: " + dbtrAgt + "\n";
            }
            data += dbtrAgt;
            
            String cdtrAgt = rs.getString("creditorAgent");
            if (cdtrAgt == null) {
                cdtrAgt = "";
            } else {
                cdtrAgt = "Creditor Agent\t\t\t: " + cdtrAgt + "\n";
            }
            data += cdtrAgt;
...
//        ResultSet rs = st.executeQuery();
        ...
        if (channel != null && !channel.isBlank()) {
            if (channel.equalsIgnoreCase("EMS")) {
                where.append(" AND source IN ('EMS', 'NCBS')");
            } else if (channel.equalsIgnoreCase("CUSTODY") || "CSA".equalsIgnoreCase(channel)) {
                where.append(" AND source IN ('CSA', 'CUSTODY')");
            } else if (channel.equalsIgnoreCase("Treasury OPS")) {
                where.append(" AND source IN ('FRONTARENA', 'TSA', 'Treasury OPS')");
            } else {
                where.append(" AND source LIKE '%" + channel + "%'");
            }
        }
        ...
        if (date_from != null && !date_from.isEmpty() && date_end != null && !date_end.isEmpty()) {
            where.append(" AND TO_CHAR(h.tanggal, 'YYYY-MM-DD HH24:MI:SS') BETWEEN ? AND ?");
            params.add(date_from + " 00:00:00");
            params.add(date_end + " 23:59:59");
        }
...
//            System.out.println(sql);
        PreparedStatement st = this.conn.prepareStatement(sql);
        log.info("PreparedStatement: " + st.toString());
