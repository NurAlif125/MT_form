...
//    String jumlah = "1";
    ...
    public void updateFIA(DataFIA data, String id) {
        try {
            String sql = "Update configuration_fia SET source=?,mtormx=?,needcheckaml=?,needconverted=?,sourceto=? where id=?";
    ...
    public List<DataFIA> getAllFIA() throws Exception {
        ...
        String sql = "SELECT id,source,mtormx,needcheckaml,needconverted,sourceto FROM configuration_fia ORDER BY id desc";
        PreparedStatement st = this.conn.prepareStatement(sql);
    ...
    public DataFIA getFiaById(String id) throws SQLException {
        DataFIA data = new DataFIA();
        String sql = "SELECT id,source,mtormx,needcheckaml,needconverted,sourceto FROM configuration_fia WHERE id='" + id + "'";
        PreparedStatement st = this.conn.prepareStatement(sql);
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
            data.setId(rs.getInt(1));
    ...
    public List<String[]> getPagesFIAAjax(int offset, int numberLimit) throws Exception {
        ...
        String sql = "SELECT id,source,mtormx,needcheckaml,needconverted,sourceto FROM configuration_fia ORDER BY id OFFSET " + offset + " ROWS FETCH NEXT " + numberLimit + " ROWS ONLY";
//        System.out.println("sql=" + sql);
        PreparedStatement st = this.conn.prepareStatement(sql);
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
