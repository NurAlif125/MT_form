...
<div class="form-row"><span class="label"><a style="color:red;text-decoration:none">*</a>Institution</span>
    <%--c:choose>
        <c:when test="${headerById.io_type == null}">
            <select name="receiver_institution" id="receiver_institution">
                <option value=""></option>
                <c:forEach var="item" items="${dataNos}">
                    <option value="${item.swift_code}" <c:if test="${item.swift_code == headerById.receiverAddress}"> selected="true" </c:if> >${item.swift_code}</option>
                </c:forEach>
            </select>
        </c:when>
        <c:when test="${headerById.io_type == 'I'}">
            <c:choose>
                <c:when test="${headerById.messageType == '103'}">
                    <select name="receiver_institution" id="receiver_institution">
                        <option value=""></option>
                        <c:forEach var="item" items="${dataNos}">
                            <option value="${item.swift_code}" <c:if test="${item.swift_code == headerById.receiverAddress}"> selected="true" </c:if> >${item.swift_code}</option>
