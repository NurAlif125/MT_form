...
function numbersonly(e, obj) {
    ...
    if (charTyped === "," && document.getElementById(obj).value.indexOf(",") !== -1) {
        e.preventDefault();
        return false; 
    }
...
function numbersOnlyNew(e, obj) {
    const charCode = e.which || e.keyCode;
    const el = document.getElementById(obj);
    var charTyped = String.fromCharCode(charCode);
    
    if (el) {
        if (el.value.toUpperCase() === "JPY") {
            if (charCode > 31 && (charCode < 48 || charCode > 57)) {
                e.preventDefault();
                return false;
            }
    ...
    if (charTyped === "," && el.value.indexOf(",") !== -1) {
        e.preventDefault();
        return false; 
    }
