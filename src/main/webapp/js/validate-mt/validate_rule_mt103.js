...
$(document).ready(function () {
        ...
        function toggleSenderChargesRequirement() {
            ...
            const empty71GFields = [
                "_230_of71g_currency", "_231_of71g_amount"
            ];

            if (chargeVal === 'BEN') {                
                const allEmpty71F = emptyFields71F.every(selector => ($(selector).val() || "").trim() === "");
                // const allEmpty71GFields = empty71GFields.every(selector => ($(selector).val() || "").trim() === "");
                ...
                if (allEmpty71F) {
                    ...
                    for (let i = 0; i < emptyFields71F.length; i += 2) {
                        const currencySelector = emptyFields71F[i];
                        const amountSelector = emptyFields71F[i + 1];
            ...
            if (chargeVal === 'SHA' || chargeVal === 'BEN') { 
                // const empty71GFields = [
                //     "_230_of71g_currency", "_231_of71g_amount"
                // ]; 
                // const allEmpty71GFields = empty71GFields.every(selector => ($(selector).val() || "").trim() === "");
                // if (!allEmpty71GFields) {
                //     alert("Please clear the fields in 71G if you select SHA or BEN in 71A");
                //     return false;
                // }

                const has71f = emptyFields71F.some((selector) => ($(selector).val() || "").trim() !== "");
                const has33bCurrency = $("#_070_of33b_currency").val().trim() !== "";
                const has33bAmount = $("#_072_of33b_amount").val().trim() !== "";

                // console.log("has71f:", has71f, "has33bCurrency:", has33bCurrency, "has33bAmount:", has33bAmount);

                if (has71f && (!has33bCurrency || !has33bAmount)) {
                    // jika 71f ada isi dan 33b kosong autofill dari 32a
                    $("#_070_of33b_currency").val($("#_061_mf32a_currency").val());
            ...
            if (chargeVal === 'OUR') {
                const currency71g = $("#_230_of71g_currency").val().trim();
                const amount71g   = $("#_231_of71g_amount").val().trim();
                console.log("currency71g:", currency71g, "amount71g:", amount71g);
                
                if (currency71g !== "" && amount71g !== "") {
                    console.log("71G ada isinya, ambil ke 33B");
                    // ambil currency 71G → isi ke 33B kalau kosong
                    if ($.trim($("#_070_of33b_currency").val()) === "") {
                        $("#_070_of33b_currency").val($("#_061_mf32a_currency").val());
                    }

                    // ambil amount (hapus koma)
                    const amount32a = $("#_062_mf32a_amount").val().trim().replace(/,/g, "");
                    const amount71g = $("#_231_of71g_amount").val().trim().replace(/,/g, "");

                    // jumlahkan
                    const hasiltambah = Number(amount32a) + Number(amount71g);

                    // simpan ke 33B amount, format sesuai kebutuhan
                    $("#_072_of33b_amount")
                        .val(hasiltambah.toString() + ",")
                        .trigger('blur');
                }
            }
        }
...
//                },
                ...
                type_of59_59a_: {required: "mf59_59a must be filled..!!"},
                ...
                _230_of71g_currency: {
                    mustBeEmptyIfCharges: "Currency must be empty if Charges is BEN or SHA"
                },
                _231_of71g_amount: {
                    mustBeEmptyIfCharges: "Amount must be empty if Charges is BEN or SHA"
                }
...
//            }
        ...
        $.validator.addMethod("mustBeEmptyIfCharges", function (value, element) {
            const chargeVal = $("#_210_of71a_details_charges").val();
            if (chargeVal === "BEN" || chargeVal === "SHA") {
                return $.trim(value) === "";  // must be empty
            }
            return true; // not BEN → ignore
        }, "This field must be empty when Charges is BEN or SHA");
    });
