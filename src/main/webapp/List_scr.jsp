...
<div id="isi">
    ...
    <form id="form1" name="form1" method="get" action="controllerHeaders?find=yes">
        ...
        <% Date tgl_today = new java.util.Date();
            ...
            String defaultTimeEnd = "23:59:59";
            pageContext.setAttribute("defaultTimeFrom", defaultTimeFrom);
            pageContext.setAttribute("defaultTimeEnd", defaultTimeEnd);
        %>
        ...
        <div id="tabs-1" class="formBody">
            ...
            <div class="form-row"><span class="labelL">I/O</span>
                <c:set var="hasIncoming" value="false" />
                 <c:set var="hasOutgoing" value="false" />
                 <!-- Loop untuk deteksi role -->
                 <c:forEach var="r" items="${role}">
                     <c:if test="${r == 'TYPE:INCOMING'}">
                         <c:set var="hasIncoming" value="true" />
                     </c:if>
                     <c:if test="${r == 'TYPE:OUTGOING'}">
                         <c:set var="hasOutgoing" value="true" />
                     </c:if>
                 </c:forEach>    
                <select name="io_type" id="io_type">
                        <c:if test="${hasIncoming and hasOutgoing}">
                            <option value="IO">All</option>
                            <option value="O"<% if("O".equalsIgnoreCase((String) session.getAttribute("io_type"))) { out.println("selected='true'"); } %>>Incoming</option>
                            <option value="I" <% if("I".equalsIgnoreCase((String) session.getAttribute("io_type"))) { out.println("selected='true'"); } %>>Outgoing</option>
                        </c:if>
                        <c:if test="${hasIncoming and not hasOutgoing}">
                            <option value="O" >Incoming</option>
                        </c:if>
                        <c:if test="${hasOutgoing and not hasIncoming}">
            ...
            <div class="form-row"><span class="labelL">Date From</span>
                <input type="text" name="date_from" id="date_from" value="<fmt:formatDate pattern='yyyy-MM-dd' value='${tgl_today}' />" size="10" /> [ex.2013-31-01] <input type="text" class="time_from" name="time_from" id="time_from" placeholder="00:00:00" size="10" value='${defaultTimeFrom}' /> [ex: 00:00:00]
            </div>
            <div class="form-row"><span class="labelL">Date End</span>
                <input type="text" name="date_end" id="date_end" value="<fmt:formatDate pattern='yyyy-MM-dd' value='${tgl_today}' />" size="10" /> [ex.2013-31-01] <input type="text" class="time_end" name="time_end" id="time_end" placeholder="23:59:59" size="10" value='${defaultTimeEnd}' /> [ex: 00:00:00]
            </div>
            <div class="form-row"><span class="labelL">Status</span>
                ...
                <%
                    String[] flags = new String[]{"MOD", "VER", "AUTH", "FIA-FAILED", "FIA-FAILED-CNF", "FIA-RESEND", "UNSETTLE-OUT", "WAITING-SAA-CNF", "WAITING-SAA-RESEND",
                        ...
                        "ACK", "NACK", "INC-CVT", "RESEND-CNF", "INC-CVT-RESEND", "INC-AML", "AML-TERMINATE-IN", "INC-AML-FAILED-CNF", "INC-AML-RESEND",
