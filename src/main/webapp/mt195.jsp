...
<%@ include file="rule/validate_rule_mt195.jsp" %>
...
<div id="isi">
    ...
    <c:forEach var="item" items="${role}">
        <c:if test="${item == 'MT:195'}">
            <div id="judul">MT 195 Queries</div>
            <form id="form1" name="form1" method="post" action="ServletControllerDataTransaksiOutgoing">
                <input name="messageType" id="messageType" type="hidden" value="195" />
                    ...
                    <div class="tabcontents">
                        ...
                        <div id="view2" class="tabcontent">
                            ...
                            <div class="form-row"><span class="label_body">MF75</span><span class="label">*Queries</span>
                                <textarea name="_012_mf75_queries" id="_012_mf75_queries"><c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_012_mf75_queries'}"><c:out value="${item.detail}" /></c:if></c:forEach></textarea>
                            </div>
                            <hr/>

                            <!-- OF77A -->
                            <div class="form-row"><span class="label_body">OF77A</span><span class="label">Narrative</span>
                                <textarea name="_013_of77a_narrative" id="_013_of77a_narrative"><c:forEach var="item" items="${tags}"><c:if test="${item.tagName=='_013_of77a_narrative'}"><c:out value="${item.detail}" /></c:if></c:forEach></textarea>
                            </div>
                            <hr/>
